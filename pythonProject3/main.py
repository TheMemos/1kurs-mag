...
# furry = ctypes.CDLL('./Libr.dll')
items = [random.randint(-100000, 100000) for i in range(10000)]
print(items)
# print(furry.sumtwo(10,5))
# print(furry.sum(items))
# print(type(items))
...
def fibonacci_search_number(arr, target):
    n = len(arr)
    fib_m_2 = 0  # (m-2)-е число Фибоначчи
    fib_m_1 = 1  # (m-1)-е число Фибоначчи
    fib_m = fib_m_2 + fib_m_1  # m-е число Фибоначчи

    # Находим такое число Фибоначчи, которое больше или равно длине массива
    while fib_m < n:
        fib_m_2 = fib_m_1
        fib_m_1 = fib_m
        fib_m = fib_m_2 + fib_m_1

    # Сдвигаем индексы так, чтобы последний элемент массива был элементом с индексом fib_m_1 - 1
    offset = -1
    while n > fib_m_1:
        n -= fib_m_1
        fib_m_1 = fib_m_2
    ...
    # Выполняем поиск с помощью метода Фибоначчи
    while fib_m > 1:
        i = offset + fib_m_2
        if arr[i] < target:
            fib_m = fib_m_1
    ...
    # Проверяем, равен ли последний элемент целевому значению
    if fib_m_1 == 1 and arr[offset] == target:
        return offset

    return None

def fibonacci_search(arr, target):
    index = fibonacci_search_number(arr, target)
    if index is not None:
        print(f"Элемент {target} найден под индексом {index}")
    else:
